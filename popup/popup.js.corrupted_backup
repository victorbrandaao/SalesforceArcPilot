document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateStatus(status = "connected") {
    const statusMap = {
      connected: { text: "Conectado", class: "status-connected", cliText: "✅ CLI Disponível" },
      loading: { text: "Carregando...", class: "status-loading", cliText: "⏳ Verificando CLI..." },
      error: { text: "Erro de Conexão", class: "status-error", cliText: "❌ CLI Indisponível" },
      offline: { text: "Offline", class: "status-offline", cliText: "⚠️ Servidor Offline" },
    };
    ...
    // Update CLI status
    if (cliStatusText) {
      cliStatusText.textContent = statusInfo.cliText;
    }
    
    if (cliStatusIndicator) {
      const cliStatusDot = cliStatusIndicator.querySelector('.status-dot');
      if (cliStatusDot) {
        cliStatusDot.className = `status-dot ${status === 'connected' ? 'online' : status === 'loading' ? 'checking' : 'error'}`;
      }
    }
  }
  ...
  async function checkServerHealth() {
    ...
    try {
      ...
      if (cliData.success && cliData.cliInstalled) {
        ...
        updateStatus("error");
        if (cliStatusText) {
          cliStatusText.textContent = "❌ Salesforce CLI não encontrado";
        }
        showToast("Salesforce CLI não está instalado ou configurado", "warning", 5000);
      }
      ...
      if (cliStatusText) {
        cliStatusText.textContent = "⚠️ Servidor local offline";
      }
      showToast("Servidor local não está rodando. Inicie o servidor para usar recursos CLI.", "error", 8000);
    }
  }
